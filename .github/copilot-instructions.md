...
- **StockAnalyse UX** : `StockAnalyse.jsx` affiche dans l'ordre — identité + cours, carte Tomino Intelligence (mémo proactif), cours & marché, avertissement source limitée, historique financier (`SectionHistorique` : CA/résultat/FCF/marges + P/E historique si disponible), valorisation, comparaison sectorielle (`SectorComparison` vs médianes hardcodées `SECTOR_BENCHMARKS`), flux de trésorerie, santé financière, dividendes, DCF interactif + grille de sensibilité (`DCFSensitivity`), consensus analystes. Le score /100 (`ScoreGauge`) est masqué du rendu mais `computeScore()` reste utilisé pour le chat.
- **Carte Tomino Intelligence** (`MemoGrok`) : fond vert dégradé + bordure verte identiques au bloc Intelligence de la sidebar (`border: rgba(24,195,126,0.26)`, `background: linear-gradient(180deg, rgba(24,195,126,0.07), rgba(24,195,126,0.02))`). Label **TOMINO INTELLIGENCE** en monospace cyan `rgba(110,231,255,0.88)` uppercase — même style que `nav-group-label` dans la sidebar. Jamais "Grok" ni "memo" dans le label visible. Générée automatiquement au chargement via `POST /api/stock/memo` → `generer_memo_action()` dans `grok.py`, non-streaming, max 1100 tokens. Rendu markdown via `.grok-memo` CSS (h2 = monospace vert uppercase + ligne verte, paragraphes espacés, em en `--text-3`).
- **Prompt mémo** (`generer_memo_action`) : chaque argument doit expliquer le POURQUOI (moat, position concurrentielle), pas juste citer un chiffre. Les données hors-norme (rendement >15%, Altman Z >30...) doivent être signalées, pas utilisées comme arguments. Risques/catalyseurs spécifiques à l'entreprise. Verdict = nommer le principal désaccord bulls/bears, pas une synthèse molle.
- **Modales info** : toujours utiliser `MetricModal` — jamais de modale `createPortal` maison. Bouton &#9432; : `color: rgba(255,255,255,0.25)` → `var(--green)` au hover. Nouvelles métriques METRIC_INFO ajoutées : `memo_grok`, `sector_comparison`, `dcf_sensitivity`.
- **Médianes sectorielles** (`SECTOR_BENCHMARKS`) : hardcodées en frontend pour 11 secteurs, pas d'API externe.
